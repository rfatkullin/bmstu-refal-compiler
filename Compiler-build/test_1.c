...
void __initLiteralData()
{
	...
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_IDENT_TAG, .str = "A"};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_IDENT_TAG, .str = "S"};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_IDENT_TAG, .str = "D"};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_IDENT_TAG, .str = "Daasd"};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 'c'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 'h'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 'e'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 'c'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 'k'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 'c'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_IDENT_TAG, .str = "asdasd"};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 's'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 'd'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 'a'};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_CHAR_TAG, .ch = 's'};
	...
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_INT_NUM_TAG, .intNum = 3};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_IDENT_TAG, .str = "Hello"};
	*(memMngr.termsHeap++) = (struct v_term){.tag = V_IDENT_TAG, .str = "Aaa"};
...
struct func_result_t Go(int entryPoint, struct env_t* env, struct field_view_t* fieldOfView) 
{
	...
	switch (entryPoint)
	{
		case 0: 
		{
			...
			helper[1]->chain->end->next = helper[7];
			helper[7]->prev = helper[1]->chain->end;
			helper[1]->chain->end = helper[7];
			helper[8]->tag = L_TERM_CHAIN_TAG;
			helper[1]->chain->end->next = helper[8];
			helper[8]->prev = helper[1]->chain->end;
			helper[1]->chain->end = helper[8];
			currTerm = (struct lterm_t*)malloc(sizeof(struct lterm_t));
			currTerm->tag = L_TERM_FRAGMENT_TAG;
			currTerm->fragment = (struct fragment_t*)malloc(sizeof(struct fragment_t));
			currTerm->fragment->offset = 22;
			currTerm->fragment->length = 2;

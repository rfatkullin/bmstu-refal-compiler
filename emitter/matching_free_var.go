...
func (f *Data) matchingFreeTermVar(depth int, ctx *emitterContext, varNumber int) {
	...
	f.PrintLabel(depth+2, fmt.Sprintf("CURR_FUNC_CALL->env->locals[%d].fragment->length = memMngr.vterms[fragmentOffset].inBracketLength;", varNumber))
	f.PrintLabel(depth+2, "fragmentOffset += memMngr.vterms[fragmentOffset].inBracketLength;")
	f.PrintLabel(depth+1, "}")

	f.PrintLabel(depth+1, "else")
	f.PrintLabel(depth+1, "{")
	f.PrintLabel(depth+2, fmt.Sprintf("CURR_FUNC_CALL->env->locals[%d].fragment->length = 1;", varNumber))
...
func (f *Data) varStretching(depth, varNumber int, ctx *emitterContext) {
	...
	if ctx.isLeftMatching {
		...
		f.PrintLabel(depth+1, "if (memMngr.vterms[fragmentOffset].tag == V_BRACKET_OPEN_TAG)")
		f.PrintLabel(depth+1, "{")
		f.PrintLabel(depth+2, fmt.Sprintf("CURR_FUNC_CALL->env->locals[%d].fragment->length += memMngr.vterms[fragmentOffset].inBracketLength;",
			varNumber))
		f.PrintLabel(depth+2, "fragmentOffset += memMngr.vterms[fragmentOffset].inBracketLength;")
		f.PrintLabel(depth+1, "}")
	...
	if ctx.isLeftMatching {
		...
		f.PrintLabel(depth+3, fmt.Sprintf("CURR_FUNC_CALL->env->locals[%d].fragment->length -= memMngr.vterms[fragmentOffset].inBracketLength;", varNumber))
		f.PrintLabel(depth+3, "fragmentOffset -= (memMngr.vterms[fragmentOffset].inBracketLength - 1);")
		f.PrintLabel(depth+2, "}")
		f.PrintLabel(depth+2, "else")
		f.PrintLabel(depth+2, "{")
		f.PrintLabel(depth+3, fmt.Sprintf("CURR_FUNC_CALL->env->locals[%d].fragment->length--;", varNumber))
		f.PrintLabel(depth+2, "}")
	}

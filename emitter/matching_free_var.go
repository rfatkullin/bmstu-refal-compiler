...
func (f *Data) matchingFreeTermVar(depth int, ctx *emitterContext, varNumber int) {
	...
	f.PrintLabel(depth, "{")
	f.PrintLabel(depth+1, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->offset = fragmentOffset;", varNumber))

	f.PrintLabel(depth+1, "if (memMngr.vterms[fragmentOffset].tag == V_BRACKET_OPEN_TAG)")
	f.PrintLabel(depth+1, "{")
	f.PrintLabel(depth+2, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length = memMngr.vterms[fragmentOffset].inBracketLength;", varNumber))
	f.PrintLabel(depth+2, "fragmentOffset += memMngr.vterms[fragmentOffset].inBracketLength;")
	f.PrintLabel(depth+1, "}")

	f.PrintLabel(depth+1, "else")
	f.PrintLabel(depth+1, "{")
	f.PrintLabel(depth+2, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length = 1;", varNumber))
...
func (f *Data) matchingFreeSymbolVar(depth int, ctx *emitterContext, varNumber int) {
	...
	f.PrintLabel(depth, "{")
	f.PrintLabel(depth+1, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->offset = fragmentOffset;", varNumber))
	f.PrintLabel(depth+1, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length = 1;", varNumber))
...
func (f *Data) matchingFreeExprVar(depth int, ctx *emitterContext, varNumber int) {
	...
	if ctx.isLeftMatching {
		f.PrintLabel(depth+1, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length = 0;", varNumber))
	} else {
		f.PrintLabel(depth+1, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length = rightCheckOffset - fragmentOffset;", varNumber))
		f.PrintLabel(depth+1, fmt.Sprintf("fragmentOffset += _currFuncCall->env->locals[%d].fragment->length;", varNumber))
	}
...
func (f *Data) varStretching(depth, varNumber int, ctx *emitterContext) {
	...
	f.PrintLabel(depth+1, fmt.Sprintf("_currFuncCall->env->stretchVarsNumber[%d] = %d;", patternNumber, ctx.patternCtx.entryPoint))
	f.PrintLabel(depth+1, "//Restore last offset at this point")

	f.PrintLabel(depth+1, fmt.Sprintf("fragmentOffset = _currFuncCall->env->locals[%d].fragment->offset + "+
		" _currFuncCall->env->locals[%d].fragment->length;", varNumber, varNumber))

	if ctx.isLeftMatching {
		f.printOffsetCheck(depth+1, prevStretchVarNumber, "")
		f.PrintLabel(depth+1, "if (memMngr.vterms[fragmentOffset].tag == V_BRACKET_OPEN_TAG)")
		f.PrintLabel(depth+1, "{")
		f.PrintLabel(depth+2, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length += memMngr.vterms[fragmentOffset].inBracketLength;",
			varNumber))
		f.PrintLabel(depth+2, "fragmentOffset += memMngr.vterms[fragmentOffset].inBracketLength;")
		f.PrintLabel(depth+1, "}")

	} else {
		f.PrintLabel(depth+1, fmt.Sprintf("if (_currFuncCall->env->locals[%d].fragment->length <= 0)", varNumber))
	...
	if ctx.isLeftMatching {
		f.PrintLabel(depth+2, "fragmentOffset++;")
		f.PrintLabel(depth+2, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length++;", varNumber))
	} else {
		f.PrintLabel(depth+2, "fragmentOffset--;")

		f.PrintLabel(depth+2, "if (memMngr.vterms[fragmentOffset].tag == V_BRACKET_CLOSE_TAG)")
		f.PrintLabel(depth+2, "{")
		f.PrintLabel(depth+3, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length -= memMngr.vterms[fragmentOffset].inBracketLength;", varNumber))
		f.PrintLabel(depth+3, "fragmentOffset -= (memMngr.vterms[fragmentOffset].inBracketLength - 1);")
		f.PrintLabel(depth+2, "}")
		f.PrintLabel(depth+2, "else")
		f.PrintLabel(depth+2, "{")
		f.PrintLabel(depth+3, fmt.Sprintf("_currFuncCall->env->locals[%d].fragment->length--;", varNumber))
		f.PrintLabel(depth+2, "}")
	}

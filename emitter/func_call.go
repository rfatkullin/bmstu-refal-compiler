...
func (f *Data) isFuncName(ident string, ctx *emitterContext) (*fk.FuncInfo, bool) {
	...
	if funcInfo, ok := ctx.funcsKeeper.IsThereFunc(ctx.fullPathKeeper.String() + ident); ok {
		return funcInfo, ok
	}
...
func (f *Data) constructFunctionalVTerm(depth int, ctx *emitterContext, funcInfo *fk.FuncInfo) {

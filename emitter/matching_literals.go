...
func (emt *EmitterData) matchingIntLiteral(depth int, ctx *emitterContext, index int) {

	emt.printLabel(depth-1, "//Matching int literal")

	emt.printOffsetCheck(depth, ctx.patternCtx.prevEntryPoint, "")
	...
	emt.printFailBlock(depth, ctx.patternCtx.prevEntryPoint, true)

	emt.printLabel(depth, "fragmentOffset++;")
}

func (emt *EmitterData) mathcingDoubleLiteral(depth int, ctx *emitterContext, index int) {
...
func (emt *EmitterData) matchingCompLiteral(depth int, ctx *emitterContext, index int) {
...
func (emt *EmitterData) matchingStrLiteral(depth int, ctx *emitterContext, strLen, index int) {
	...
	emt.printFailBlock(depth+1, ctx.patternCtx.prevEntryPoint, true)
